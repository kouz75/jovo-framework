...
class GoogleAction extends Platform {
  ...
  constructor(jovo) {
    ...
    if (util.getRequestType(this.request) === 'GoogleActionDialogFlowV2Request' ||
      ...
      this.response = new GoogleActionResponseV2(null, this.request.getSession(), this.getUserId());
    } else {
  ...
  getUserId() {
    ...
    try {
      const storage = JSON.parse(_.get(this.request, 'originalDetectIntentRequest.payload.user.userStorage'));
      storageUserId = storage.userId
    } catch (e) {
    }
    const newUserId = storageUserId || userId || uuidv4();
    _.set(this, 'user.userId', newUserId);
    return newUserId;
  }
