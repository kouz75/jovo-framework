...
/**
 * GoogleActionResponseV2 Class
 */
class GoogleActionResponseV2 {
    /**
     * Constructor
     * @param {*=} responseObj
     * @param {*=} session
     */
    constructor(responseObj, session, userId) {
        if (responseObj) {
            this.responseObj = responseObj;
        } else {
            this.responseObj = {
                fulfillmentText: '<speak></speak>',
                payload: {
                    google: {
      ...
      this.responseObj.payload.google.userStorage = JSON.stringify({userId: userId});
      if (session) {
            this.contextPath = session + '/contexts/';
        }
    }

    /**
     * Returns a outputContexts object by the given name
     * @param {string} name
     * @return {*}
     */
    getContextOut(name) {
        if (!this.responseObj.outputContexts) {
            return {};
        }
        for (let i = 0; i < this.responseObj.outputContexts.length; i++) {
            if (this.responseObj.outputContexts[i].name.toLowerCase() === name.toLowerCase()) {
                return this.responseObj.outputContexts[i];
            }
        }
        return {};
    }
    ...
    /**
     * Sets outputContexts object. Overwrites existing objects with same name
     * @param {*} contextObj
     */
    setContextOut(contextObj) {
        if (!this.responseObj.outputContexts || this.responseObj.outputContexts.length === 0) {
            this.addContextOutObject(contextObj);
        } else {
            for (let i = 0; i < this.responseObj.outputContexts.length; i++) {
                if (this.responseObj.outputContexts[i].name.toLowerCase() ===
                    contextObj.name.toLowerCase()) {
                    this.responseObj.outputContexts[i] = contextObj;
                }
            }
        }
    }
    ...
    setContextOutArray(contexts) {
        this.responseObj.outputContexts = contexts;
    }
    ...
    addContextOutObject(context) {
        if (!this.responseObj.outputContexts) {
            this.responseObj.outputContexts = [];
        }
        this.responseObj.outputContexts.push(context);
    }
    ...
    setContextOutParameters(contextName, parameters) {
        contextName = this.contextPath + contextName;
    ...
    getSessionAttributes() {
        const contextName = this.contextPath + 'session';
        return this.getContextOut(contextName).parameters;
    }
     ...
     */
    getPermissions() {
        return _.get(this, 'responseObj.payload.google.permissions_request.permissions', []);
    }
     ...
     */
    tell(speech) {
        this.responseObj.fulfillmentText = speech;
        this.responseObj.payload.google.expectUserResponse = false;
        let editedSimpleResponse = false;
        for (let item of this.responseObj.payload.google.richResponse.items) {
        ...
        if (!editedSimpleResponse) {
            this.responseObj.payload.google.richResponse
                .items.unshift(
                {
                    simpleResponse: {
                        ssml: speech,
    ...
    ask(speech, repromptSpeech) {
        this.tell(speech);
        this.responseObj.fulfillmentText = speech;
        this.responseObj.payload.google.expectUserResponse = true;
        this.responseObj.payload.google.noInputPrompts = [];
        for (let noInputPrompt of repromptSpeech) {
            this.responseObj.payload.google.noInputPrompts.push({
                ssml: noInputPrompt,
            });
        }
        return this;
     ...
     */
    getSpeechText() {
        if (this.responseObj.payload.google.richResponse.items[0]
            .simpleResponse.ssml) {
            return removeSpeakTags(this.responseObj.payload
    ...
    addBasicCard(title, formattedText) {
        this.responseObj.payload.google.richResponse.items.push(
            {
    ...
    addAccountLinkingCard() {
        const previousInputs = this.responseObj.payload.google.expectedInputs || [];
        previousInputs.push(getSignInInput());

        this.responseObj.payload.google.expectedInputs = previousInputs;
        return this;
    }
    ...
    addSuggestionChips(chips) {
        _.set(this.responseObj, 'payload.google.richResponse.suggestions', []);
        for (let chip of chips) {
            this.responseObj.payload.google.richResponse.suggestions.push({
                title: chip,
            });
        }
        return this;
    ...
    addLinkOutSuggestion(destinationName, url) {
        ...
        _.set(this.responseObj, 'payload.google.richResponse.linkOutSuggestion', linkOutSuggestion);
        return this;
    }
    ...
    addList(list) {
        ...
        _.set(this.responseObj, 'payload.google.systemIntent', systemIntent);
        return this;
    }
    ...
    addAskForPermission(permissions, speech) {
        ...
        this.responseObj.fulfillmentText = speech;
        _.set(this.responseObj, 'payload.google.expectUserResponse', true);
        let systemIntent = {
    ...
    addAskForSignIn() {
        this.responseObj = {
            ...
            'outputContext': this.responseObj.outputContext,
        };
        return this;
    }
    ...
    isTell(speechText) {
        try {
            if (this.responseObj.payload.google.expectUserResponse !== false) {
                return false;
            }
            ...
            if (speechText) {
                if (speechText.constructor === Array) {
                    let filteredSpeechText = speechText.filter((text) => {
                        return this.responseObj.fulfillmentText === toSSML(text) &&
                            this.responseObj.payload.google.richResponse
                    ...
                    if (this.responseObj.fulfillmentText !== toSSML(speechText)) {
                        return false;
                    }
                    if (this.responseObj.payload.google.richResponse
    ...
    isAsk(speechText, repromptText) {
        try {
            if (this.responseObj.payload.google.expectUserResponse !== true) {
                return false;
            }
            ...
            if (repromptText) {
                if (repromptText.constructor === Array) {
                    let filteredSpeechText = speechText.filter((text) => {
                        return this.responseObj.payload.google.noInputPrompts[0].ssml ===
                            toSSML(text);
                    });
                    if (filteredSpeechText.length === 0) {
                        return false;
                    ...
                    if (this.responseObj.payload.google.noInputPrompts[0]
    ...
    hasBasicCard(title, formattedText) {
        try {
            let items = this.responseObj.payload.google.richResponse.items;
    ...
    hasSessionAttributes() {
        return this.hasSessionAttribute();
    }
...
function setPermissionToResponse(permission) {
    ...
    _.set(this, 'responseObj.payload.google.permissions_request.permissions', permissionArray);
    return this;
}
...
module.exports.GoogleActionResponseV2 = GoogleActionResponseV2;
...
module.exports.GoogleActionResponseV2.CardBuilder = CardBuilder;
